...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            ...
            bot.lavalink.add_node('yourhostport', 2333, 'yourpassword', 'na', 'default-node')  # Host, Port, Password, Region, Name
    ...
    async def jump_to(self,msg,track_time:int):
        player = self.bot.lavalink.player_manager(msg.guild.id)
        if track_time > player.duration:
            track_time = player.duration
